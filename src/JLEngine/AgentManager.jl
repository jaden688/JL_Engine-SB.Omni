...
AgentManager(root_dir::AbstractString=pwd(), agents_dir::AbstractString="agents") = AgentManager(String(root_dir), String(agents_dir), nothing, Dict{String, Any}(), nothing, 0.5)

function set_active_agent!(manager::AgentManager, name::AbstractString, data::AbstractDict, registry::Union{Nothing, Dict{String, MPFProfile}}=nothing)
    manager.active_name = String(name)
    manager.base_data = Dict{String, Any}(string(key) => value for (key, value) in pairs(data))
    manager.secondary_data = registry === nothing ? nothing : _find_related_agent(manager, String(name), registry)
    manager.dynamic_trait_weight = 0.5
    return manager
end

function _find_related_agent(manager::AgentManager, name::String, registry::Dict{String, MPFProfile})
    ...
    for (display_name, profile) in registry
        ...
        agent_path = resolve_path(manager.root_dir, joinpath(manager.agents_dir, profile.agent_file))
        isfile(agent_path) || continue
        candidate = load_agent_file(agent_path)
        candidate isa AbstractDict && return Dict{String, Any}(string(key) => value for (key, value) in pairs(candidate))
    end
    return
end

function apply_supervisor_bias!(manager::AgentManager, bias::Real)
    manager.dynamic_trait_weight = clamp(manager.dynamic_trait_weight + Float64(bias) * 0.25, 0.0, 1.0)
    return manager
end

function update_dynamic_weight!(manager::AgentManager, signals=nothing; rhythm_state=nothing, aperture_state=nothing)
...
function get_projection(manager::AgentManager)
    agent = deepcopy(manager.base_data)
    agent["dynamic_trait_weight"] = round(manager.dynamic_trait_weight; digits=3)
    if manager.secondary_data !== nothing && manager.dynamic_trait_weight > 0.05
        ...
        agent["operational_behavioral_traits"] = Dict{String, Any}(
